...
test('create an investment via the UI', async ({ page }) => {
  const uniqueNote = `e2e-investment-${Date.now()}`;

  await page.goto('/transactions/new');
  await page.getByRole('button', { name: 'Investment' }).click();

  await expect(page.getByRole('button', { name: /save investment/i })).toBeVisible();
  ...
  await pickFromDropdown(
    page,
    page.locator('button:has-text("Select investment account")').first(),
    /Zerodha/i,
  );
